...
class App extends Component {
  ...
  componentDidMount() {
    ...
    allInstituteList.forEach((institute) => {
      const instituteString = institute
        .toString()
        .replace(/,/g, "")
        .slice(0, -4);
      let urlunencoded = `(((${instituteString})) AND ("`;
      // let urlunencoded = `(((Olivia Newton-John Cancer Research Institute)) AND ("`
      ...
      axios
        .get(url)
        .then((response) => {
          const idlistAll = this.state.idlistAll.concat(
            response.data.esearchresult.idlist
          );
          this.setState({ idlistAll }, () => {
            loop = loop + 1;
          });
          allInstituteList.length === loop && this.addPapers();
        })
  ...
  dateInput = (date1, date2) => {
    if (!this.state.selectedInstitute) {
      alert(
        "The date input does not work if all Institutes are selected. Go to the drop-down menu to select an institute before you input a date."
      );
    } else {
  ...
  getIdList = () => {
    ...
    if (!this.state.inputedDate1) {
      dateParams = `${this.state.dateMinus7}"[Date - Entrez] : "3000"[Date - Entrez])`;
    ...
    axios
      ...
      .then((response) => {
        this.setState(
          { idlistSelected: response.data.esearchresult.idlist },
          () => {
            this.addPapers();
          }
        );
      ...
      if (this.state.idlistSelected.length > 2000) {
        alert(
          "Your search returned: " +
            this.state.idlistSelected.length +
  ...
  resetDates = () => {
    this.setState({ inputedDate1: "", inputedDate2: "" }, () =>
      this.getIdList()
    );
  };
              ...
              <>
                ...
                <div className="displayContainer">
                  {this.state.selectedInstitute ? (
                    <div>
                      <h1 className="instituteTitle">
                        {this.state.selectedInstitute}
                      </h1>
                      <p className="instituteTitleSubHeading">
                        Customise search dates above
                      </p>
                    </div>
                  ) : (
                    <div>
